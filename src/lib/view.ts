...
export class viteView implements IViewEngine {
  ...
  async getSsrHtml(indexName:string,entryServerUrl:string,url:string,assign:object|undefined) {
    ...
    try {
      ...
      if (!this.prod) {
        ...
        manifest = require(this.staticFileConfig.dirs.default.dir + '/html/ssr-manifest.json');
  ...
  async renderString(tpl: string,locals?: Record<string, any>,options?: RenderOptions) {
    tpl = path.resolve( options.root,tpl);
    if(this.viteViewConfig.prod !== undefined){
      this.prod = this.viteViewConfig.prod
    }else {
      this.prod = process.env.MIDWAY_SERVER_ENV == 'prod' || process.env.NODE_ENV=='prod';
    }
